...
function Post() {
    ...
    const handleSubmit = async(event)=>{
        ...
        try {
            const response = await axios.post(`http://localhost:5000/Post`, postForm, config);
    ...
    <>
      <div>
        ...
        <form action='http://localhost:5000/Post' method='POST' enctype = "multipart/form-data" target="dummyframe">
          ...
          <input type="text" name="desc" id="desc" onChange={handleChange}></input>
          <br />
          <label>Category</label>
          <input type="text" name="cat" id="cat" onChange={handleChange}></input>
          <br />
